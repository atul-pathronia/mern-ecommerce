...
export const getProducts =
  ...
  async (dispatch) => {
    try {
      ...
      let link = `${api.endpoint}/products?keyword=${keyword}&page=${currentPage}&price[gte]=${price[0]}&price[lte]=${price[1]}&rating[gte]=${rating}`;

      if (category) {
        link = `${api.endpoint}/products?keyword=${keyword}&page=${currentPage}&price[gte]=${price[0]}&price[lte]=${price[1]}&category=${category}&rating[gte]=${rating}`;
      }
...
export const getProductsAdmin = () => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(`${api.endpoint}/products/admin/products`);
    dispatch({
...
export const createProductAdmin = (productData) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.post(
      `${api.endpoint}/products/admin/product/new`,
      productData,
      config
    );
    dispatch({
...
export const updateProductAdmin = (id, productData) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.put(
      `${api.endpoint}/products/admin/product/${id}`,
      productData,
      config
    );
    dispatch({
...
export const deleteProductAdmin = (id) => async (dispatch) => {
  try {
    ...
    await axios.delete(`${api.endpoint}/products/admin/product/${id}`);
    dispatch({
